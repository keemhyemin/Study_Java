...
public class Ex03_LinkedList {
	public static void main(String[] args) {
		// 사용빈도가 낮음. 인덱스끼리 체이닝(연결) 구조를 가지고있어서 장점이 한두가지 있지만
		...
		for(int i=0; i<10000; i++) {
			listArray.add(i+""); // 0번 인덱스에 끼워넣기 처리를 함. Linked가 끼워넣기에는 더 빠르다
		}
		...
		startTime = System.nanoTime();
		for(int i=0; i<10000; i++) {
			listLinked.add(i+""); // 0번 인덱스에 끼워넣기 처리를 함. Linked가 끼워넣기에는 더 빠르다
		}
		endTime = System.nanoTime();
		System.out.println("ArrayList의 작업시간: " + (endTime - startTime) + "ns");
